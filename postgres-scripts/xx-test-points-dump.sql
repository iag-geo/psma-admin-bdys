...
--   FROM admin_bdys_201902.locality_bdys_display_full_res
...
SELECT Count(*) as num_points FROM (
 ...
 FROM admin_bdys_201902.locality_bdys
) AS sqt;
...
DROP TABLE IF EXISTS admin_bdys_201902.test_locality_points;
WITH pnts AS (
  SELECT locality_pid,
...
SELECT pnts.locality_pid,
  ...
  INTO admin_bdys_201902.test_locality_points
FROM pnts;

CREATE INDEX test_locality_points_pid_idx ON admin_bdys_201902.test_locality_points USING btree (locality_pid); -- 4:10
ANALYZE admin_bdys_201902.test_locality_points;