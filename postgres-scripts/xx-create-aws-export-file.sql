...
SELECT locality_pid, locality_name, postcode, state, ST_AsBinary(geom) AS geom FROM admin_bdys_202605.locality_bdys_display;



psql -U postgres -c "COPY (SELECT locality_pid, locality_name, postcode, state, ST_AsBinary(geom) AS geom FROM admin_bdys_202605.locality_bdys_display) TO stdout DELIMITER '|'" geo | gzip > locality_bdys_display.psv.gz
...
LOCATION
  's3://minus34.com/opendata/geoscape-202605/admin_bdys'
