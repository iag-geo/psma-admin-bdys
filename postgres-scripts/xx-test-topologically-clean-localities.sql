...
CREATE TABLE testing.locality_bdys_display AS
  SELECT locality_pid, ST_RemoveRepeatedPoints(geom) AS geom FROM admin_bdys_202111.locality_bdys_display;
...
-- Total points
SELECT SUM(ST_NPoints(geom)) FROM admin_bdys_202111.locality_bdys;         -- 12,938,094
SELECT SUM(ST_NPoints(geom)) FROM admin_bdys_202111.locality_bdys_display; --  4,392,196 (66% reduction)
